...
/**
 * A {@link ManagedDependenciesResolver} that resolves versions using Maven Resolver.
 *
 * @author Andy Wilkinson
 */
class MavenResolverManagedDependenciesResolver implements ManagedDependenciesResolver {
	...
	MavenResolverManagedDependenciesResolver(File cacheLocation) {
		ServiceLocator serviceLocator = createServiceLocator();
		DefaultRepositorySystemSession session = MavenRepositorySystemUtils.newSession();
		session.setArtifactDescriptorPolicy(new SimpleArtifactDescriptorPolicy(false, false));
		LocalRepository localRepository = new LocalRepository(cacheLocation);
